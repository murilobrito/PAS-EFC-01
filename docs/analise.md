...
## 8.2 (b) Solu��es Implementadas

*   **SRP**: Separa��o de responsabilidades em OrderService (c�lculos e gest�o do pedido) e ReportService (gera��o de relat�rios).
*   **ISP**: Cria��o da interface abstrata OrderRepositoryInterface que obriga as classes concretas a implementar apenas os m�todos de persist�ncia.
*   **DIP**: O OrderService agora depende de OrderRepositoryInterface e das estrat�gias de pagamento e desconto, recebendo-as via construtor (inje��o de depend�ncias). O Sis instancia e injeta essas depend�ncias.
*   **OCP**: A implementa��o dos padr�es Strategy e Observer permite adicionar novos m�todos de pagamento, descontos e notifica��es sem modificar as classes principais de servi�o.
*   **LSP**: A classe filha PedEspecial que quebrava o contrato foi substitu�da por uso direto do OrderFactory e de OrderDiscountStrategy, garantindo total consist�ncia.
